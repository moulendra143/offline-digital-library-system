...
if (isset($_POST['submit'])) {
    ...
    if (isset($_FILES['pdf_file']['name'])) {
        ...
        if (strtolower($file_extension) == 'pdf') {
            move_uploaded_file($file_tmp, "./generalbooks/" . $file_name);

            $insertquery = "INSERT INTO generalbooks(id,username, filename, author, code) VALUES($i,'$name', '$file_name','$author', '$code')";
            $iquery = mysqli_query($con, $insertquery);

            if ($iquery) {
                echo "File uploaded and data inserted successfully.";
                header("location:viewgeneralbooks.php");
            } else {
...
if (isset($_GET['query'])) {
    ...
    $selectquery = "SELECT * FROM generalbooks WHERE code LIKE ?";
